...
const dataset = async () => {
  ...
  // Utiliser la structure originale du modèle stat
  await Stat.create({
    quantite_st: 2,
    date_st: new Date(),
    commande_st: "CMD001",
  ...
  // Ajouter quelques statistiques supplémentaires pour les tests
  await Stat.create({
    quantite_st: -5,
    date_st: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), // Il y a 7 jours
    commande_st: "CMD002",
  })

  await Stat.create({
    quantite_st: -3,
    date_st: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000), // Il y a 15 jours
    commande_st: "CMD003",
  })
